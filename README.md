...
> Webpack4.x+Babel7.x+React16.x+Typescript 平台搭建

## 项目初始化

通过`npm init`初始化项目，一路 yes，生成 package.json 文件
...
- 全局安装`@babel/cli` `@babel/core`，安装后可检查 babel 版本
...
- 在根目录下新建 config 目录，新建如下三个配置文件：

  - webpack.base.conf.js 为公用配置
  ...
  - webpack.dev.conf.js 为开发环境配置，继承自 webpack.base.conf.js
  ...
  - webpack.prod.conf.js 为生产环境配置，继承自 webpack.base.conf.js
